...
import { createBrandVoiceSamples } from "@/lib/services/user-preferences"
import { z } from "zod"
...
const voiceSamplesSchema = z.object({
  samples: z.array(voiceSampleSchema).min(1),
})
...
export const POST = createRouteHandler(
  async (request: NextRequest, { auth }) => {
    ...
    const body = await parseJson(request, voiceSamplesSchema)
    const samples = await createBrandVoiceSamples(auth.userId, body.samples)
