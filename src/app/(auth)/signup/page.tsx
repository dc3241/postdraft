...
export default function SignupPage() {
  ...
  const onSubmit = async (data: SignupForm) => {
    ...
    try {
      ...
      const result = await api.auth.signup(data.email, data.password, data.fullName)
      ...
      if (!result.session) {
        ...
        setTimeout(() => {
          window.location.href = "/login?message=Please check your email to confirm your account"
        }, 1500)
        return
      }

      // Session exists - use full page reload to ensure cookies are properly read by middleware
      toast.success("Account created successfully!")
      // Use window.location.href to force full page reload and ensure cookies are set
      setTimeout(() => {
        window.location.href = "/onboarding/niches"
      }, 500)
    } catch (error) {
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-lg border border-stone-200 p-8">
        ...
        <form onSubmit={handleSubmit(onSubmit, onError)} className="space-y-5">
          <div className="space-y-2">
            <Label htmlFor="fullName" className="text-sm font-medium text-stone-700 mb-2">
              Full Name (Optional)
            </Label>
            <Input
              id="fullName"
